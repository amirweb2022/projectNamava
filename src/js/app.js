...
function startCarousel1() {
  if (counter1 >= imageCount) 
  counter1 = 0;
  carouselpro.style.transform = "translateX(" + imageSize * counter1 + "px)";
  counter1++;
}
...
function getdatapro() {
    ...
    xhr.onload = function () {
      ...
      if (xhr.status === 200) {
        ...
        data.forEach((data) => {
            output1 += `<a href="#" class="text-decoration-none">
            <div class="box-movie position-relative mx-2">
                <img src="${data.imgmovie}" class="rounded">
                <div class="overlay">
                    <p class="text-light">${data.overtitle}</p>
        ...
        carouselProVizhe1.innerHTML = output1;
      }
    };
    xhr.send();
}